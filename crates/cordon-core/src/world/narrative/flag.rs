...
mod tests {
    ...
    #[test]
    fn untagged_flag_value_parses_each_variant() {
        let n: QuestFlagValue = serde_json::from_str("42").unwrap();
        assert!(matches!(n, QuestFlagValue::Number(v) if v == 42.0));

        let b: QuestFlagValue = serde_json::from_str("true").unwrap();
        assert!(matches!(b, QuestFlagValue::Boolean(true)));

        let s: QuestFlagValue = serde_json::from_str(r#""hello""#).unwrap();
        assert!(matches!(s, QuestFlagValue::String(v) if v == "hello"));
    }
    ...
    #[test]
    fn predicate_is_set_parses() {
        let p: QuestFlagPredicate = serde_json::from_str(r#""is_set""#).unwrap();
        assert!(matches!(p, QuestFlagPredicate::IsSet));
    }
}
