...
pub struct ItemDef {
    ...
    /// (e.g., `"longtooth"`, `"medkit"`, `"rifle_ammo"`).
    pub id: Id<Item>,
    /// Type-specific data: consumable effects, weapon caliber, etc.
    pub data: ItemData,
    /// Base price with no market modifiers. Supplier-specific
    /// prices multiply this by the supplier's `price_multiplier`.
    pub base_price: Credits,
    /// NPC templates that act as suppliers for this item. Each
    /// referenced template must have its own `supplier: Some(...)`
    /// declared. An empty list means the item isn't player-orderable
    /// (e.g. weapons, armor — earned/looted rather than bought).
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub suppliers: Vec<Id<NpcTemplate>>,
