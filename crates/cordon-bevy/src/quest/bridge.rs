...
//!   Idempotency is handled by the quest's own
//!   [`ActiveQuest::awaiting_dialogue`] flag.
...
//!   variables into the quest's flag bag, and calls
//!   [`engine::advance_after_talk`] with whatever
//!   `$quest_choice` the Yarn node wrote.
...
/// that hasn't enqueued a visitor yet, push one onto the
/// bunker's [`VisitorQueue`] and mark the quest as awaiting
/// dialogue. Runs every frame — the `awaiting_dialogue` flag
/// keeps it idempotent.
